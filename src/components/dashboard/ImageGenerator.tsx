...
interface ImageGeneratorProps {
  onGenerateSuccess: (originalImage: string, generatedImage: string, prompt: string, provider: string) => void;
  freeGenerationsLeft?: number;
  subscriptionTier?: string;
}

const DEFAULT_PROMPT = 'Transform this image into Studio Ghibli style artwork with soft pastel colors, detailed natural elements, and whimsical characters';
...
export default function ImageGenerator({
  ...
  const [provider, setProvider] = useState(userPreferences.defaultProvider || 'openai');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (freeGenerationsLeft <= 0 && subscriptionTier === 'free' && !hasProviderKey) {
        throw new Error(`You've used all your free generations. Please subscribe or enter a custom ${provider} API key in your profile.`);
      }
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3 }}>
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <FormControl fullWidth sx={{ mb: 3 }}>
            ...
            >
              ...
              <MenuItem value="stability">{getProviderLabel('stability')}</MenuItem>
              <MenuItem value="midjourney">{getProviderLabel('midjourney')}</MenuItem>
              <MenuItem value="leonardo">{getProviderLabel('leonardo')}</MenuItem>
            </Select>
            <FormHelperText>
              {hasProviderKey 
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
            Free Generations Left: <Chip size="small" label={freeGenerationsLeft} color={freeGenerationsLeft > 0 ? "primary" : "default"} />
          </Typography>
        </Box>
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Button
            ...
            size="large"
            fullWidth
            startIcon={generating ? <CircularProgress size={20} color="inherit" /> : <MagicWandIcon />}
          ...
          <Box sx={{ mt: 3 }}>
            ...
            <Typography variant="caption" color="text.secondary">
              Your image will be transformed using AI to match the Ghibli art style.
